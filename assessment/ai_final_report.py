...
이후 학습률 {step_size}로 이동을 반복한 결과, {steps} step 이동 후 최종점 {end_pt}에 도달하였다. 이때 최종 손실값은 {final_e} 이었다. 이 결과를 통해 편미분을 이용한 이동 방향 판단이 손실값 감소와 어떻게 연결되는지 확인할 수 있다. 다만 손실곡면의 형태가 복잡하거나 학습률이 적절하지 않은 경우에는 경로가 진동하거나 국소적인 낮은 지점에 머물 수 있으므로, 경사하강법이 항상 전역 최소점에 도달한다고 단정할 수는 없다.
""".strip()
...
3. 경사하강법을 사용할 때 학습률 손실곡면의 형태를 함께 고려해야 하는 이유는 무엇인가?
