...
def build_step2_backup(payload: dict) -> bytes:
    ...
    lines.append("")
    lines.append("[파이썬 수식(3차시용)]")
    lines.append("- py_model (f(t)):")
    lines.append(payload.get("py_model", "").strip() or "(미입력)")
    lines.append("")
    lines.append("- py_d1 (f'(t)):")
    lines.append(payload.get("py_d1", "").strip() or "(미입력)")
    lines.append("")
    lines.append("- py_d2 (f''(t)):")
    lines.append(payload.get("py_d2", "").strip() or "(미입력)")
    lines.append("")    
...
4) 파이썬 계산용 식: numpy를 사용한 한 줄 코드로 작성 (예: 3.2 * np.exp(0.04 * t))
   (변수는 반드시 t를 사용하고, 모델식은 f, 도함수는 d1, 이계도함수는 d2라는 키워드로 제공할 것)
...
st.info(
    ...
    "모델 식으로부터 도출한 도함수 $f'(t)$가 이를 얼마나 잘 설명하는지 서술하시오.\n\n"
    "🔹 곡선의 모양 분석\n\n"
    "데이터의 이계변화율($\\Delta^2 y/\\Delta t^2$) 그래프에 나타난 오목·볼록 상태를 "
    "모델 식으로부터 도출한 이계도함수 $f''(t)$와 비교하여 분석하시오.\n\n"
    "🔹 모델의 한계\n\n"
    "실제 데이터와 모델 식의 차이가 큰 구간 또는 시점을 한 곳 제시하고, "
