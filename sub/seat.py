...
def init_db():
    with get_conn() as conn:
        ...
        # ✅ track(과목), class_letter(분반) 컬럼 포함
        conn.execute(
            """
            CREATE TABLE IF NOT EXISTS assignments (
                ...
                class_letter TEXT,                       -- 분반(A,C,D,J,B,F,H 등)
                track TEXT,                              -- 과목(미적분/AI수학)
...
def list_assignments() -> dict:
    """Returns dict seat_id -> {user_token, student_id, student_name, class_letter, track, assigned_at}"""
    with get_conn() as conn:
        rows = conn.execute(
            "SELECT seat_id, user_token, student_id, student_name, class_letter, track, assigned_at FROM assignments;"
        ).fetchall()
    out = {}
    for seat_id, user_token, student_id, student_name, class_letter, track, assigned_at in rows:
        out[str(seat_id)] = {
            "user_token": user_token,
            "student_id": str(student_id),
            "student_name": student_name,
            "class_letter": (class_letter or ""),
            "track": (track or ""),
...
def get_user_assignment(user_token: str):
    with get_conn() as conn:
        row = conn.execute(
            "SELECT seat_id, student_id, student_name, class_letter, track, assigned_at FROM assignments WHERE user_token = ?;",
            (user_token,),
        ).fetchone()
    if not row:
        return None
    return {
        ...
        "student_name": row[2],
        "class_letter": (row[3] or ""),
        "track": (row[4] or ""),
        "assigned_at": row[5],
    }


def try_assign(seat_id: str, user_token: str, student_id: str, student_name: str, class_letter: str, track: str):
    """
    ...
    try:
        with get_conn() as conn:
            ...
            conn.execute(
                """
                INSERT INTO assignments(seat_id, user_token, student_id, student_name, class_letter, track, assigned_at)
                VALUES(?, ?, ?, ?, ?, ?, ?);
                """,
                (str(seat_id), user_token, str(student_id), student_name, class_letter, track, now),
            )
            conn.execute("COMMIT;")
        return {"ok": True}
...
def validate_track_and_class(track: str, class_letter: str) -> tuple[bool, str]:
    track = (track or "").strip()
    cl = normalize_class_letter(class_letter)
    if not track:
        return False, "과목을 선택하세요."
    if not cl:
        return False, "분반을 입력하세요. (예: A, C, J)"
    if track == "미적분" and cl not in CALC_CLASSES:
        return False, "미적분 분반은 A, C, D, J 입니다."
    if track == "AI수학" and cl not in AI_MATH_CLASSES:
        return False, "AI수학 분반은 B, F, H 입니다."
    return True, ""
...
with tab_student:
    ...
    track = st.selectbox(
        "과목 선택",
        TRACK_OPTIONS,
        index=TRACK_OPTIONS.index(st.session_state.get("track", "미적분"))
        if st.session_state.get("track", "미적분") in TRACK_OPTIONS
        else 0,
    )
    st.session_state.track = track

    class_letter_in = st.text_input("분반(예: A, C, J)", value=st.session_state.get("class_letter", ""))
    if class_letter_in:
        st.session_state.class_letter = normalize_class_letter(class_letter_in)

    ok_tc, msg_tc = validate_track_and_class(st.session_state.get("track", ""), st.session_state.get("class_letter", ""))
    if not ok_tc:
        st.info(msg_tc)
    ...
    # ✅ 입력 조건에 과목/분반 포함
    if (
        not st.session_state.get("student_id")
        or not st.session_state.get("student_name")
        or not st.session_state.get("track")
        or not st.session_state.get("class_letter")
        or not ok_tc
    ):
        st.info("학번, 이름, 과목, 분반을 입력하면 대기 상태로 들어갑니다.")
    else:
        if not is_open:
                    ...
                    if cols[c - 1].button(label, key=f"btn_{seat_id}_{round_id}", disabled=disabled):
                        if not my_seat:
                            result = try_assign(
                                ...
                                class_letter=st.session_state.class_letter,
    ...
    if (
        ...
        and st.session_state.get("track")
        and st.session_state.get("class_letter")
    ):
        st.write(
            f"- 학번 이름: {st.session_state.student_id} {st.session_state.student_name} "
            f"({st.session_state.track}/{st.session_state.class_letter})"
        )
        if my:
...
with tab_teacher:
    ...
    if not assignments:
        ...
        for seat_id, info in items:
            tag = ""
            if info.get("track") or info.get("class_letter"):
                tag = f"[{info.get('track','')}/{info.get('class_letter','')}] "
            st.write(f"- **{seat_id}** : {tag}{info['student_id']} {info['student_name']}")
