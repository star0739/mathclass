...
# (추가) 테스트 페이지 등록
# -----------------------------
# NOTE: 추후 테스트 페이지를 없앨 계획이면,
# 1) 아래 test_page 정의 부분을 삭제(또는 주석 처리)하고
# 2) pages 딕셔너리의 "🧪 테스트" 섹션을 삭제(또는 주석 처리)하고
# 3) 사이드바의 "Test" 버튼 블록을 삭제(또는 주석 처리)하면 됩니다.
test_page = st.Page(
    "test/test.py",   # test 폴더 아래 test.py
    title="Test",
    icon="🧪",
)
...
pages = {
    ...
    # (추가) 좌석관리 하위가 아니라, 별도의 테스트 메뉴로 분리
    # NOTE: 추후 제거하려면 이 줄(섹션)만 지우면 메뉴에서 사라집니다.
...
with st.sidebar:
  ...
  # -----------------------------
    # (추가) 테스트 메뉴
    # -----------------------------
    # NOTE: 추후 테스트 페이지 제거 시 아래 블록을 통째로 삭제(또는 주석 처리)하면 됩니다.
    st.markdown("---")
    st.subheader("🧪 테스트")
    if st.button("Test", use_container_width=True, key="sb_test"):
        # 페이지 객체 대신 "경로 문자열"로 이동하는 것이 가장 안정적입니다.
